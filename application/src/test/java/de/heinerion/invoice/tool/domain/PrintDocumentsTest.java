 ...
 * </p>
 * <p>
 * This test only assures that something is triggered to be written to the filesystem, not exactly what it is and how it
 * looks like
 * </p>
 */
...
public class PrintDocumentsTest {
  ...
  @Before
  public void setUp() {
    fileService = EasyMock.createNiceMock(FileService.class);
  ...
  private Capture<String> prepareTextCapture() {
    Capture<String> textCapture = EasyMock.newCapture();
    EasyMock.expect(fileService.writeTex(EasyMock.eq("Path"), EasyMock.eq("file"), EasyMock.capture(textCapture))).andReturn(true);
    EasyMock.expect(fileService.texToPdf("Path", "file")).andReturn(true);
    replay(fileService);
    return textCapture;
  }
}
